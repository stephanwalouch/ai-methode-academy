...
-- ============================================================
-- ADMIN-ROLLE SETZEN (direkt nach dem Setup)
-- ============================================================
-- Warten bis Trigger das Profil angelegt hat und dann Rolle setzen.
-- Da der Nutzer info@ai-methode.de bereits in auth.users existiert,
-- wird sein Profil sofort beim Schema-Run angelegt (via Trigger-Backfill).

-- Profil sicherstellen (falls Trigger noch nicht gelaufen)
INSERT INTO profiles (id, email, role)
SELECT id, email, 'admin'
FROM auth.users
WHERE email = 'info@ai-methode.de'
ON CONFLICT (id) DO UPDATE SET role = 'admin';

-- Bestätigung
SELECT id, email, role FROM profiles WHERE email = 'info@ai-methode.de';
